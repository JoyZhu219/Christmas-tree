...
export default function PolaroidPhotos({ progress, isMobile }) {
  ...
  const photoCount = 12
  const { gl } = useThree() // 获取渲染器实例，为了读取硬件能力
  ...
  useEffect(() => {
    textures.forEach(texture => {
      // 1. 开启各向异性过滤 (Anisotropy)
      // 这会让照片在侧面看时依然非常锐利，不会模糊
  ...
  const photosData = useMemo(() => {
    // ... (保留之前的 Chaos/Formed 位置计算逻辑，完全不需要变) ...
    // 为了节省篇幅，这里省略中间的计算代码，请保留你原文件中 photosData 的内容
    const data = []
    for (let i = 0; i < photoCount; i++) {
        ...
        const chaosRot = new THREE.Euler((Math.random() - 0.5) * 0.3, (Math.random() - 0.5) * 0.3, (Math.random() - 0.5) * 0.2)
        ...
        const targetPos = new THREE.Vector3(Math.cos(angle) * radius, treeY, Math.sin(angle) * radius)
        const targetRot = new THREE.Euler(-0.1, -angle + Math.PI / 2, (Math.random() - 0.5) * 0.15)
      ...
      {photosData.map((data, i) => (
        <group key={i}>
            ...
            <mesh position={[0, 0.1, 0.02]}>
                ...
                <meshBasicMaterial 
                  map={textures[i]} 
                  toneMapped={false} // 关键：防止被后期 Bloom 泛白，保持原本的对比度
                />
            </mesh>
            {/* 玻璃层 (仅 PC) */}
            {!isMobile && (
